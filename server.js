...
app.get('/tokens', async (req, res) => {
	...
	try {
		...
		let tokenItems = tokens.map((t) => t.symbol).splice(0,500)

		if (tokenItems.length) {
			// coinGecko URL to get dynamic currency record
			const coinGeckoUrl = `https://api.coingecko.com/api/v3/simple/price?ids=${tokenItems.join(',')}&vs_currencies=${currency}`
